...
namespace libtui.drawing
{
	...
	public sealed class GraphicsPath : ICloneable, IDisposable, IGeometry
	{
		...
		static int FlattenPath(GraphicsPath path, float flatness)
		{
			var status = 0;
		...
		public RectangleF GetBounds(Pen pen)
		{
			...
			var status = FlattenPath(workPath, 25.0f);
		...
		{
			return BoundaryTracing.IsInside(x, y, PathPoints);
		}
		...
		{
			Warp(destPoints, srcRect, WarpMode.Perspective);
		}

		public void Warp(PointF[] destPoints, RectangleF srcRect, WarpMode warpMode)
		{

			Warp(destPoints, srcRect, WarpMode.Perspective, 0.25f);

		}

		public void Warp(PointF[] destPoints, RectangleF srcRect, WarpMode warpMode, float flatness)
		{
			...
			if (destPoints.Length == 3)
			{
				...
				GeomTransformUtils.WarpPath(this, destPoints1, srcRect, warpMode, flatness);
			}
			else
			{
				GeomTransformUtils.WarpPath(this, destPoints, srcRect, warpMode, flatness);
			}
		}
        ...
        {
			int index = 0;
			yield return new GeometryData(GeometryDataType.Vertex, PathPoints.Select(x => new GeometryPoint(index++, x, 0)).ToArray());
        }
