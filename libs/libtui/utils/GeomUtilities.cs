...
namespace libtui.utils
{
    internal static class GeomUtilities
	{
		...
		internal static void ComputeOrientationLine (RectangleF rect, float angle, out PointF start, out PointF end)
		{
			...
			if (angle < 90) {
				start = new PointF(rect.Location.X, rect.Location.Y);
				tanSize.Width = rect.Size.Width;
				tanSize.Height = rect.Size.Height;
			} else if (angle < 180) {
				start = new PointF(rect.Location.X + rect.Size.Width, rect.Location.Y);
				tanSize.Width = -rect.Size.Width;
				tanSize.Height = rect.Size.Height;
			} else if (angle < 270) {
				start = new PointF(rect.Location.X + rect.Size.Width, rect.Location.Y + rect.Size.Height);
				tanSize.Width = -rect.Size.Width;
				tanSize.Height = -rect.Size.Height;
			} else {
				start = new PointF(rect.Location.X, rect.Location.Y + rect.Size.Height);
				tanSize.Width = rect.Size.Width;
				tanSize.Height = -rect.Size.Height;
			}


			float radAngle = MathTools.ToRadians(angle); 
			...
			end = new PointF((float)Math.Cos(radAngle) * distance + start.X, (float)Math.Sin(radAngle) * distance + start.Y);
		}
		...
		internal static PointF [] GetCurveTangents (int terms, PointF [] points, int count, float tension, CurveType type)
		{
			...
			for (int i = 0; i < count; i++) {
				...
				tangents[i] = new PointF(tangents[i].X + (coefficient * (points[r].X - points[s].X)), tangents[i].Y + (coefficient * (points[r].Y - points[s].Y)));
			}
		...
		internal static void QuadraticToCubic(PointF start, PointF controlPoint, PointF end, out PointF controlPoint1, out PointF controlPoint2)
		{
			controlPoint1 = new PointF(
				start.X + (quadCubeCoeff * (controlPoint.X - start.X)),
				start.Y + (quadCubeCoeff * (controlPoint.Y - start.Y)));

			controlPoint2 = new PointF(
				end.X + (quadCubeCoeff * (controlPoint.X - end.X)),
				end.Y + (quadCubeCoeff * (controlPoint.Y - end.Y)));
		}
		...
		internal static float Normal(PointF v)
		{
		...
		internal static PointF PolygonCentroid(PointF[] points)
		{
			...
			for (int i = 1; i < points.Length; i++)
			{
				...
				area += dotProd;
				C = new PointF(C.X + (last.X + next.X) * dotProd, C.Y + (last.Y + next.Y) * dotProd);
				last = next;
			}
			...
			C = new PointF(C.X + (last.X + first.X) * dotProd, C.Y + (last.Y + first.Y) * dotProd);
			...
			C = new PointF(C.X * reciprocal, C.Y * reciprocal);
			return C;
		}
