...
{
    abstract class EnviromentLibrary
    {
        ...
        protected EnviromentLibrary(string libraryName)
        {
            _LibraryName = libraryName;
            _handle = GetLibraryNameCandidates()
                .Select(LoadLibrary)
                .FirstOrDefault(handle => handle != IntPtr.Zero);
            if (_handle == IntPtr.Zero)
                throw new NotImplementedException($"{libraryName} native library was not found.");
        }

        public TDelegate GetStaticProc<TDelegate>(string procName)
        ...
        private IEnumerable<string> GetLibraryNameCandidates()
        {
            if (IsOSPlatform(Windows))
            {
                return GetWindowsLibraries();
            }
            else if (IsOSPlatform(Linux))
            {
                return GetLinuxLibraries();
            }
            else if (IsOSPlatform(OSX))
            {
                return GetOSXLibraries();
            }
            throw new NotImplementedException($"Ran out of places to look for the {_LibraryName} native library.");
        }

        protected string GetPlatformIdentity()
        {
            if (IsOSPlatform(Windows))
            {
                return $"win-x{(Environment.Is64BitProcess ? 64 : 86)}";
            }
            else if (IsOSPlatform(Linux))
            {
                return $"linux-x{(Environment.Is64BitProcess ? 64 : 86)}";
            }
            else if (IsOSPlatform(OSX))
            {
                return $"osx-x{(Environment.Is64BitProcess ? 64 : 86)}";
            }
            throw new NotImplementedException($"{_LibraryName} not support for {Environment.OSVersion} platform.");
        }

        protected abstract IEnumerable<string> GetWindowsLibraries();

        protected abstract IEnumerable<string> GetLinuxLibraries();

        protected abstract IEnumerable<string> GetOSXLibraries();
