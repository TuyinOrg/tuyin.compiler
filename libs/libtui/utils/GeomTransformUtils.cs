...
namespace libtui.utils
{
	internal static class GeomTransformUtils
	{
		...
		public static void WarpPath(GraphicsPath path, PointF[] destPoints, RectangleF srcRect,  WarpMode warpMode = WarpMode.Perspective, float flatness = 0.25f)
		{
			...
			if (warpMode == WarpMode.Perspective)
			{
				...
				for (i = 0; i < count; i++)
				{
					...
					pnts[i] = new PointF(
						(float)(factor * (coeffs[0] * x1 + coeffs[1] * y1 + coeffs[2])),
						(float)(factor * (coeffs[3] * x1 + coeffs[4] * y1 + coeffs[5])));
				}
			}
			else
			{
				...
				for (i = 0; i < count; i++)
				{
					...
					pnts[i] = new PointF(
						(float)(coeffs[0] * x1 + coeffs[1] * y1 + coeffs[2] * x1 * y1 + coeffs[3]),
						(float)(coeffs[4] * x1 + coeffs[5] * y1 + coeffs[6] * x1 * y1 + coeffs[7]));
