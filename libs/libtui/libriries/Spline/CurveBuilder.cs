...
using VECTOR = libtui.drawing.PointF;
...
namespace Spline
{
    ...
    /// </summary>
    sealed class CurveBuilder : CurveFitBase, IEnumerable<CubicBezier>
    {
        ...
        /// <summary>
        /// Adds a data curve to the curve builder.
        /// </summary>
        public AddPointResult AddPoint(VECTOR c1, VECTOR c2, VECTOR np)
        {
            List<VECTOR> pts = _pts;
            int count = pts.Count;
            if (count != 0)
            {
                ...
                VECTOR p0 = pts[last - 1];
                CubicBezier curve = new CubicBezier(p0, c1, c2, np);
                FLOAT len = curve.GetArcLength();
                FLOAT alpha = len / 3;
                _arclen.Add(_totalLength = _totalLength + len);

                _tanL = (c1 - p0) / alpha;
                _result.Add(curve);
                _first = _arclen.Count - 1;
                return new AddPointResult(last == 1 ? 0 : _result.Count - 2, true);
            }
            else 
            {
                _prev = np;
