...
namespace Spline
{
    ...
    sealed class SplineBuilder
    {
        ...
        public bool Add(VECTOR c1, VECTOR c2, VECTOR np)
        {
            CurveBuilder.AddPointResult res = _builder.AddPoint(c1, c2, np);

            return Update(res);
        }

        private bool Update(CurveBuilder.AddPointResult res)
        {
            if (!res.WasChanged)
                return false;
        ...
        internal VECTOR? GetLastPoint()
        {
            if (_builder.Points.Count > 0)
                return _builder.Points[_builder.Points.Count - 1];

            if (_spline.Curves.Count > 0)
                return _spline.Sample(1);
