...
namespace Tuyin.IR.Analysis.Utils
{
    ...
    static class FastBuffer
    {
        ...
        {
            fixed (byte* src_ptr = src, dst_ptr = dst)
                UnsafeBlockCopyRL(src_ptr, dst_ptr, count);
        }
        ...
        {
            fixed (byte* src_ptr = src, dst_ptr = dst)
                UnsafeBlockCopyLR(src_ptr, dst_ptr, count);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public unsafe static void ParallelBlockCopyRL(byte[] src, byte[] dst, int count)
        {
            fixed (byte* src_ptr = src, dst_ptr = dst)
                ParallelBlockCopyRL(src_ptr, dst_ptr, count);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public unsafe static void ParallelBlockCopyLR(byte[] src, byte[] dst, int count)
        {
            fixed (byte* src_ptr = src, dst_ptr = dst)
                ParallelBlockCopyLR(src_ptr, dst_ptr, count);
        }
