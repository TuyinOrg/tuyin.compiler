...
{
	public interface IObjectItem<TParameter>
	{
	...
	/// </typeparam>
	public class ObjectPool<T, TParameter> where T : IObjectItem<TParameter>, new()
	{
		/// <summary>
		...
		/// different pool type
		/// </summary>
		private static ObjectPool<T, TParameter> me = new ObjectPool<T, TParameter>();
		/// <summary>
		/// Using a member for the max pool count allows different
		/// types to have a different value based on usage analysis
		/// </summary>
		private int mMaxPool = ObjectPool<T, TParameter>.MAX_POOL;
		/// <summary>
		...
		/// </returns>
		public static ObjectPool<T, TParameter> GetInstance()
		{
			return me;
		}
		/// <summary>
		...
		/// </exception>
		internal void Free(object sender)
		{
		...
		/// </returns>
		public T GetObjectFromPool(TParameter parameter)
		{
			T result = default(T);
#if DISABLE_POOL
			result = new T();
