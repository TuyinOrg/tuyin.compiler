...
using compute.vulkan.ext;
using compute.vulkan.khr;
using compute.vulkan.mvk;
using System;
using System.Collections.Generic;
using System.Diagnostics;
using System.Linq;
using Semaphore = compute.vulkan.Semaphore;
...
namespace compute.environment
{
    ...
    abstract class VulkanApp : IApp
    {
        ...
        protected internal DebugReportCallbackExt DebugReportCallback { get; private set; }
        protected internal SurfaceKhr Surface { get; private set; }
        protected internal SwapchainKhr Swapchain { get; private set; }
        protected internal Image[] SwapchainImages { get; private set; }
        ...
        public virtual void Initialize(IAppHost host)
        {
            ...
            if(host.Platform == Platform.MacOS || host.Platform == Platform.IOS)
            {
        ...
        private Instance CreateInstance(bool debug)
        {
            ...
            switch (Host.Platform)
            {
                ...
                case Platform.MacOS:
                    ...
                    break;
                case Platform.IOS:
                    surfaceExtension = Constant.InstanceExtension.MvkIOSSurface;
                    break;
                default:
            ...
            if (debug && Host.Platform != Platform.MacOS && Host.Platform != Platform.IOS)
            {
        ...
        {
            //Currently MoltenVK (used for MacOS) doesn't support the debug layer.
            if (!debug || Host.Platform == Platform.MacOS || Host.Platform == Platform.IOS) return null;
        ...
        private SurfaceKhr CreateSurface()
        {
            ...
            switch (Host.Platform)
            {
                ...
                case Platform.IOS:
                    return Instance.CreateIOSSurfaceMvk(new IOSSurfaceCreateInfoMvk());
                default:
                    throw new NotImplementedException();
            }
        }
