...
namespace compute.environment.imaging
{
	...
	/// <see cref="compute.environment.imaging.FreeImage.FindFirstMetadata(FREE_IMAGE_MDMODEL, FIBITMAP, out FITAG)"/>
	/// function and then used on subsequent calls to
	/// <see cref="compute.environment.imaging.FreeImage.FindNextMetadata(FIMETADATA, out FITAG)"/>.
	/// When the <b>FIMETADATA</b> handle is no longer used, it needs to be freed by the
	/// <see cref="compute.environment.imaging.FreeImage.FindCloseMetadata(FIMETADATA)"/> function.
	/// </remarks>
