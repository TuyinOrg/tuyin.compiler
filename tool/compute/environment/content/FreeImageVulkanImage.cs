...
using static compute.environment.imaging.FreeImage;
...
namespace compute.environment.content
{
    internal static partial class Loader
    {
        ...
        public unsafe static VulkanImage LoadUncompressedVulkanImage(IAppHost host, VulkanContext ctx, IImage image) 
        {
            var format = sPixelVulkanFormat[image.PixelFormat];
            var width = image.Size.Width;
            var height = image.Size.Height;
            var depth = (int)format >> 8;
            ...
            mipmap.Data = image.Compile().ToArray();
            data.Mipmaps[0] = mipmap;

            return VulkanImage.Texture2D(ctx, data);
        }

        public unsafe static VulkanImage LoadUncompressedVulkanImage(IAppHost host, VulkanContext ctx, string path)
        {
            var dib = LoadEx(path);
        ...
        public static Format GetVulkanFormat(IImage image) => sPixelVulkanFormat.ContainsKey(image.PixelFormat) ? sPixelVulkanFormat[image.PixelFormat] : Format.Undefined;
